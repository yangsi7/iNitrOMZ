...
saveas(gcf, '../Plots/O2_NO3_POC_N2O.eps','epsc');
...
saveas(gcf, '../Plots/NO2_NH4_N2_N2Ored.eps','epsc'); 
...
saveas(gcf, '../Plots/Cremin.eps','epsc');
...
saveas(gcf, '../Plots/NO3_NO2_N2O_NH4_SMS.eps','epsc');
...
martin0p85 = @(z) bgc.poc_top*bgc.wsink(1).*(z/bgc.ztop).^(-0.85);
martin0p35 = @(z) bgc.poc_top*bgc.wsink(1).*(z/bgc.ztop).^(-0.35);
hold on;grid on;
